$path = "g:\Meine Ablage\Berufsschule\Lernseite\learning-space-main\wirtschaft.html"
$content = Get-Content -Path $path -Raw
$pattern = "(?s)<!-- TAB 1: THEORIE -->(.*)<!-- Footer -->"
$match = [regex]::Match($content, $pattern)
if ($match.Success) {
    $body = $match.Groups[1].Value
    $body = $body -replace 'class="tab-content active"', 'class="tab-pane active"'
    $body = $body -replace 'class="tab-content"', 'class="tab-pane"'
    $newContent = @"
<!DOCTYPE html>
...
"@
    ...
    Write-Output "Could not find body."
}
